    ...
    if (password == confirmPassword) {
      ...
      axios
          ...
          .catch((error) => {
            ...
            if(error.toString() == "AxiosError: Request failed with status code 400") {
              setErrorMessage("*All fields are required*")
            }

            if(error.toString() == "AxiosError: Request failed with status code 409") {
  ...
  return (
    <div className={`${resets.clapyResets} ${classes.root}`}>
      ...
      <div className={classes.username_textbox}>
        <input name="username_input" value = {username} onChange={input => setUsername(input.target.value)} style={{ width:"255.4289px", outline: "none", marginLeft: "10px", marginRight: "10px"}} maxLength = "20"/>
      </div>
      <div className={classes.password}>Password</div>
      <div className={classes.password_textbox}>
        <input name="password_input" value = {password} onChange={input => setPassword(input.target.value)} style={{ width:"255.4289px", outline: "none", marginLeft: "10px", marginRight: "10px"}} maxLength = "20"/>
      </div>
      <div className={classes.confirm_password}>Confirm Password</div>
      <div className={classes.confirm_password_textbox}>
        <input name="confirm_password_input" value = {confirmPassword} onChange={input => setConfirmPassword(input.target.value)} style={{ width:"255.4289px", outline: "none", marginLeft: "10px", marginRight: "10px"}} maxLength = "20"/>
      </div>
